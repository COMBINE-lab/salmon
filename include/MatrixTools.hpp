...
// taucs.h will define min/max macros if it's not already done (e.g. by
// Windows.h).
#ifndef min
...
/**
*  This function collapses the rate matrix "A" and the corresponding vector of
read
...
void collapseIntoCategories(
    std::vector<std::vector<double>>& A, std::vector<double>& counts,
  ...
  std::unique_ptr<std::vector<std::vector<double>>> captr(
      new std::vector<std::vector<double>>(
          numRows, std::vector<double>(numCategories, 0.0)));
  collapsedA = std::move(captr);

  std::unique_ptr<std::vector<double>> ccptr(
      new std::vector<double>(numCategories, 0.0));
  collapsedCounts = std::move(ccptr);

  size_t j = 0;
  for (auto catIt = categoryCounts.begin(); catIt != categoryCounts.end();
       ++catIt, ++j) {
...
std::vector<double> LARSSolve(MatT& Ain, VecT& bin) {
  ...
  vigra::linalg::prepareColumns(
      A, A, offset, scaling,
      vigra::linalg::DataPreparationGoals(vigra::linalg::ZeroMean |
                                          vigra::linalg::UnitVariance));
  vigra::linalg::prepareColumns(
      b, b, vigra::linalg::DataPreparationGoals(vigra::linalg::ZeroMean));
  ...
  int numSolutions = vigra::linalg::leastAngleRegression(
      A, b, activeSets, solutions,
  ...
  for (vigra::MultiArrayIndex k = 0; k < numSolutions; ++k) {
    ...
    for (unsigned int i = 0; i < activeSets[k].size(); ++i) {
      ...
      // activeSets[k][i] is the true index of the i-th variable in the active
      // set
...
std::vector<double> shotgunSolve(MatT& A, VecT& b) {
  ...
  shotgun_data prob;
  // std::cerr << "trying to reserve " << A.nonZeros() << " nonzeros in prob
  // rows/cols\n";
  ...
  for (int k = 0; k < A.outerSize(); ++k) {
    ...
    for (typename MatT::InnerIterator it(A, k); it; ++it) {
      ...
      try {
        ...
        ++ctr;
        if (ctr > nnz) {
          std::cerr << "only reserved space for " << nnz
                    << " elements, but you're pushing on the " << ctr
                    << "th element\n";
        }
...
std::vector<double> nnlsSolve(MatT& A, VecT& b) {
  ...
  nnls(ADense.data(), mda, m, n, &b[0], &x[0], &rnorm, &w[0], &zz[0], &indx[0],
       &mode);
  ...
  if( x == NULL ) {
    ...
    for( size_t i = 0; i < myX.size(); ++i ) { myX[i] = x[i]; std::cerr << "x["
  << i << "]  = " << x[i] << "\n"; }
...
template <typename MatT> std::vector<size_t> markDuplicateColumns(MatT& M) {
 ...
 */
template <typename VecT> VecT projectOnto(VecT& v, VecT& u) {
  return (v.dot(u) / u.dot(u)) * u;
}

template <typename ColT>
bool classicalGramSchmidt(std::vector<ColT>& basis,
                          typename ColT::Scalar tol = 1e-10) {
...
template <typename ColT>
bool addColumnToBasis(std::vector<ColT>& basis, ColT& c,
                      typename ColT::Scalar tol = 1e-10) {
...
template <typename MatT> std::vector<size_t> markDependentColumns(MatT& M) {
  ...
  std::sort(heights.begin(), heights.end(),
            [](const ColHeightT& a, const ColHeightT& b) -> bool {
  ...
  if (!isIndependent) {
    std::cerr << "Something went horribly wrong; the original set of vectors "
                 "was not linearly independent!\n";
