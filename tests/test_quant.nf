...
process quantSim {
  ...
  file "sim_quants/${cond}_${sample}" into simqs
	...
	if (cond == 'A')
	    """
      ${params.salmon} quant -p 16 -i ${index} -l A -1 ${basepath}_${sample}/sample_01_1_shuffled.fa.gz -2 ${basepath}_${sample}/sample_01_2_shuffled.fa.gz -o sim_quants/${cond}_${sample} --useVBOpt --vbPrior 1e-5 --validateMappings --minScoreFraction 0.95 
  	    """
	else
	    """
      ${params.salmon} quant -p 16 -i ${index} -l A -1 ${basepath}_${sample}/sample_02_1_shuffled.fa.gz -2 ${basepath}_${sample}/sample_02_2_shuffled.fa.gz -o sim_quants/${cond}_${sample} --useVBOpt --vbPrior 1e-5 --validateMappings --minScoreFraction 0.95 
	    """
}
