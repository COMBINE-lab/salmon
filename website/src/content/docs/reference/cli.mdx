...
Commands:
  ...
  degnorm     Correct read counts for transcript degradation across samples
...
| --- | --- |
| selective alignment (default or `--online`) | 50 |
| sketch, paired-end — the 2.6.0 default | 9 |
| sketch with `--online`, or single-end reads | 10 |

The deterministic mapping pass does less work per fragment (no online
inference, eq-class assembly, bias collection or prefix library-type
detection), so decoding is worth dedicated slots sooner. Since 2.6.0 that is
the default, which is why a plain `--sketch` run engages at 9; the 10 now
applies only under the deprecated `--online` path, or to single-end input,
where the cheap pass does not apply.
...
| `--degCoverage` | Accumulate per-transcript coverage curves and write `aux_info/coverage.gz`, the input to [`salmon degnorm`](#salmon-degnorm). Reads mode only. |
| `--degCovBins` | Position bins per transcript for `--degCoverage` (default 100). Costs `(bins + 1) x transcripts x 8` bytes during mapping. |
...
Correct read counts for transcript degradation, using the coverage curves
`salmon quant --degCoverage` recorded. Degradation is only identifiable by
comparison, so this takes a cohort — at least two samples quantified against the
same index — and writes a degradation index per transcript per sample plus
degradation-adjusted counts.

```sh
salmon degnorm --quants quants/s1 quants/s2 quants/s3 -o degnorm
```

| Option | Description |
| --- | --- |
| `--quants` | Quantification directories, one per sample (each needs `aux_info/coverage.gz` and, unless `--noCounts`, `quant.sf`). |
| `--names` | Sample names for the output columns (default: the directory basenames). |
| `-o/--output` | Output directory: `quants/<sample>/quant.sf` (corrected, one per sample), plus `degradation_index.tsv`, `counts_raw.tsv`, `counts_adjusted.tsv` and `degnorm_summary.json`. |
| `--minLength` | Skip transcripts shorter than this (default 200). |
| `--minCoverage` | Skip transcripts whose mean cohort coverage depth is below this (default 0.1). |
| `--maxAdjustDI` | Ceiling on the index used to inflate counts; the reported index is untouched (default 0.9). |
| `--maskFrac` | Hold entries below this fraction of the fitted envelope out of the envelope fit, as damaged (default 0.5; `0` fits every entry). |
| `--maxIter`, `--tol` | Per-transcript fit iteration cap and convergence tolerance. |
| `--noCounts` | Report degradation only; do not read `quant.sf` or adjust counts. |
| `--noQuantSf` | Write the matrices but not the per-sample corrected `quant.sf` files. |
| `-p/--threads` | Worker threads (0 = all cores). |

The corrected `quant.sf` files are drop-in replacements for the originals, so an
existing tximport/DESeq2 pipeline only needs its path changed:
...
The index is a comparative measure with a coverage-dependent positive floor, and
it differs from [DegNorm](https://nustatbioinfo.github.io/DegNorm/)'s numbers by
construction (transcripts rather than genes, posterior-weighted coverage rather
than primary alignments). Both points, and the model itself, are covered in
`docs/degnorm.md`.
