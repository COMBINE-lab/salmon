...
| `--spliced` | Report mapping output in genome coordinates instead of transcript ones, with `N` operations at exon junctions. Needs `--annotation` for the exon structures and `--genome` for the chromosome lengths the header declares. Adds `XS:A` for the transcript's strand. |
