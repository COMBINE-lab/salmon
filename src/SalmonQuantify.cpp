    ...
    while (parser->refill(rg)) {
        ...
        for (size_t i = 0; i < rangeSize; ++i) { // For all the read in this batch
            ...
            if (jointHits.size() > 0) {
                ...
                auto getAlnScore = [&aligner, &ez](int32_t pos, const char *rptr, int32_t rlen, char *tseq,
                                                   int32_t tlen, uint32_t buf) -> int32_t {
                    int32_t s{-1};
                    if (pos < 0) {
                        rptr += -pos;
                        pos = 0;
                ...
                if (tryAlign) {
                    ...
                    for (auto &h : jointHits) {
                        ...
                        if (h.mateStatus == rapmap::utils::MateStatus::PAIRED_END_PAIRED) {
                            ...
                            if ((s1 + s2) <
                                (optFrac * a * rp.first.seq.length() + optFrac * a * rp.second.seq.length())) {
                                s1 = -1000;
                                s2 = -1000;
                            }
                            score = s1 + s2;
                    ...
                    jointHits.erase(
                            std::remove_if(jointHits.begin(), jointHits.end(),
                                           [&ctr, &scores, &numDropped, /*&salmonOpts, &rp, &rc1, &rc2, &transcripts,*/ bestScore](
                                                   const QuasiAlignment &qa) -> bool {
                                               /*
...
int salmonQuantify(int argc, char *argv[]) {
    ...
    all.add(inputOpt).add(basicOpt).add(mapSpecOpt).add(advancedOpt).add(fmdOpt).add(testingOpt).add(hiddenOpt).add(
            deprecatedOpt);
