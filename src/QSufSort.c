/* QSufSort.c
   ...
   Changes summary:	- Used long variable and function names
                    ...
                    - Used insertion sort in place of selection sort and
  increased insertion sort threshold
                    - Reconstructing suffix array from inverse becomes an option
                    - Add handling where end-of-text symbol is not necessary <
  all characters
                    - Removed codes for supporting alphabet size > number of
  characters
...
// Static functions
static void QSufSortSortSplit(qsint_t* __restrict V, qsint_t* __restrict I,
                              const qsint_t lowestPos, const qsint_t highestPos,
                              const qsint_t numSortedChar);
static qsint_t QSufSortChoosePivot(qsint_t* __restrict V, qsint_t* __restrict I,
                                   const qsint_t lowestPos,
                                   const qsint_t highestPos,
                                   const qsint_t numSortedChar);
static void QSufSortInsertSortSplit(qsint_t* __restrict V,
                                    qsint_t* __restrict I,
                                    const qsint_t lowestPos,
                                    const qsint_t highestPos,
                                    const qsint_t numSortedChar);
static void QSufSortBucketSort(qsint_t* __restrict V, qsint_t* __restrict I,
                               const qsint_t numChar,
                               const qsint_t alphabetSize);
static qsint_t QSufSortTransform(qsint_t* __restrict V, qsint_t* __restrict I,
                                 const qsint_t numChar,
...
void QSufSortSuffixSort(qsint_t* __restrict V, qsint_t* __restrict I,
                        const qsint_t numChar, const qsint_t largestInputSymbol,
                        const qsint_t smallestInputSymbol,
  ...
  if (!skipTransform) {
    ...
    newAlphabetSize =
        QSufSortTransform(V, I, numChar, largestInputSymbol,
                          smallestInputSymbol, numChar, &numSymbolAggregated);
  ...
  while ((qsint_t)(I[0]) >= -(qsint_t)numChar) {
    ...
    do {
      ...
      if (s < 0) {
        ...
        if (negatedSortedGroupLength) {
          I[i + negatedSortedGroupLength] =
              negatedSortedGroupLength; /* combine preceding sorted groups */
          negatedSortedGroupLength = 0;
        }
        j = V[s] + 1;
    ...
    if (negatedSortedGroupLength) {
      ...
      I[i + negatedSortedGroupLength] =
          negatedSortedGroupLength; /* combine sorted groups at end of I.*/
    }
...
static qsint_t QSufSortTransform(qsint_t* __restrict V, qsint_t* __restrict I,
  ...
  c = maxNumInputSymbol;
  for (a = 0;
       a < numChar && maxSymbolInChunk <= maxSymbol && c <= maxNewAlphabetSize;
       a++) {
    minSymbolInChunk =
        (minSymbolInChunk << maxNumBit) | (V[a] - smallestInputSymbol + 1);
    maxSymbolInChunk = c;
    c = (maxSymbolInChunk << maxNumBit) | maxNumInputSymbol;
  }

  mask = (1 << (a - 1) * maxNumBit) -
  ...
  for (i = a; i <= numChar; i++) {
    ...
    c = ((c & mask) << maxNumBit) | (V[i] - smallestInputSymbol +
  ...
  for (i = 0, j = a; j <= numChar; i++, j++) {
    ...
    c = ((c & mask) << maxNumBit) | (V[j] - smallestInputSymbol +
                                     1); /* shift in next old symbol in chunk.*/
  }
