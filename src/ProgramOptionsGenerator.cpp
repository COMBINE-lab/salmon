...
namespace salmon {
  ...
  po::options_description ProgramOptionsGenerator::getMappingSpecificOptions(SalmonOpts& sopt) {
    ...
    mapspec.add_options()
      ...
      ("maxSoftclipFraction",
       po::value<vector<double>>(&(sopt.maxSoftclipFraction))->multitoken(),
       "[selective-alignment mode only] : The maximum soft-clipped fraction of the read length a mapping "
       "is allowed to have. This parameter accepts up to two values representing the general and overhanging "
       "soft-clip fractions respectively. If only one value is given, the same soft-clip fraction will be used "
       "in both overhanging and general soft-clipping situations. --- values should be in [0,0.8].\n"
      )
      ("computeCIGAR", 
  ...
  po::options_description ProgramOptionsGenerator::getAlignmentInputOptions(SalmonOpts& sopt) {
    ...
    ("targets,t", po::value<std::string>(),
     "FASTA format file containing target transcripts.")
      ("ont", po::bool_switch(&(sopt.oxfordNanoporeModel))->default_value(false)->notifier([&sopt](const bool& v) { if(v) sopt.noLengthCorrection = true; }),
