...
ErrorModel::ErrorModel(double alpha, uint32_t maxExpectedReadLen)
    : maxExpectedLen_(maxExpectedReadLen),
      ...
      isEnabled_(true), maxLen_(0), burnedIn_(false) {}
...
ErrorModel::logLikelihood(bam_seq_t* read, Transcript& ref,
  ...
  // salmon::stringtools::strand readStrand = (BAM_FREVERSE & read->core.flag) ?
  // salmon::stringtools::strand::reverse :
  ...
  while (basesChecked < numInc) {
    ...
    double qual = (useQual) ? salmon::stringtools::phredToLogProb[qval]
                            : salmon::math::LOG_1;
...
void ErrorModel::update(const UnpairedRead& hit, Transcript& ref, double p,
                        double mass) {
  ...
  if (uTranscriptIdx < transcriptLen) {
    ...
    if (numMismatch > 3) {
        ...
        std::cerr << "read of length " << readLen << " had " << numMismatch
                  << " mismatches; dir is " << ((readStrand == strand::forward)
    ? "forward" : "reverse") << "\n"; int32_t transcriptStartIdx =
    bam_pos(read); size_t rstart = (transcriptStartIdx < 0) ?
    -transcriptStartIdx : 0; if (transcriptStartIdx < 0) { transcriptStartIdx =
    0; } if (op == BAM_CSOFT_CLIP) { rstart += opLen;
       }
       size_t numInc = std::min(transcriptLen - transcriptStartIdx, readLen -
    rstart); int32_t rinc{1}; std::cerr << "read name is " << bam_name(read) <<
    "\n"; std::cerr << "readStart = " << rstart << "\n"; std::cerr << "txp start
    = " << transcriptStartIdx << " (txp len = " << ref.RefLength << "), (txp
    name = " << ref.RefName << ")\n"; std::cerr << "read is: "; int32_t rp =
    rstart; for (int32_t ri = 0; ri < numInc; ri++, rp++) { std::cerr <<
    samCodeToChar[bam_seqi(qseq, rp)]; } std::cerr << "\n"; std::cerr << "ref
    is:  ";
...
void ErrorModel::update(const ReadPair& hit, Transcript& ref, double p,
                        double mass) {
  if (mass == salmon::math::LOG_0) {
    return;
  }
