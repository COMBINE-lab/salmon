...
Once all the above requirement are satisfied, alevin can be run using the following command:
   ...
   > ./salmon alevin -lISR -1 cb.fastq.gz -2 reads.fastq.gz --chromium  -i salmon_index_directory -p 10 -o alevin_output --tgMap txp2gene.tsv
...
Similarly, both of these approaches can be adopted if the files are gzipped as well::
   ...
   > salmon alevin -l IISR -1 lib_A_cb.fq.gz lib_B_cb.fq.gz -2 lib_A_read.fq.gz lib_B_read.fq.gz
