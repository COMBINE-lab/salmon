...
#ifndef __INFO_MAIN_CMDLINE_HPP__
...
public:
  ...
  info_main_cmdline(int argc, char* argv[]) :
    skip_flag(false),
    json_flag(false),
    cmd_flag(false),
    file_arg("")
  ...
  void parse(int argc, char* argv[]) {
    ...
    static const char *short_options = "hVUsjc";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    ...
    if(json_flag && skip_flag)
      error("Switches [-j, --json] and [-s, --skip] are mutually exclusive");
    if(cmd_flag && skip_flag)
      error("Switches [-c, --cmd] and [-s, --skip] are mutually exclusive");
    if(cmd_flag && json_flag)
      error("Switches [-c, --cmd] and [-j, --json] are mutually exclusive");
  ...
  static const char * usage() { return "Usage: jellyfish info [options] file:path"; }
  class error {
    int code_;
