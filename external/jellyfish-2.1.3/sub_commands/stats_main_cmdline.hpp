...
#ifndef __STATS_MAIN_CMDLINE_HPP__
#define __STATS_MAIN_CMDLINE_HPP__
...
public:
  ...
  stats_main_cmdline(int argc, char* argv[]) :
    recompute_flag(false),
    lower_count_arg(0), lower_count_given(false),
    upper_count_arg(), upper_count_given(false),
    verbose_flag(false),
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
  ...
  static const char * usage() { return "Usage: jellyfish stats [options] db:path"; }
  class error {
    int code_;
